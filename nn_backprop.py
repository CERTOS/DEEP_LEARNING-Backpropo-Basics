...
def main():
  print("\nStarting Nural network's back-propagation \n")
  numInput = 4
  numHidden = 5
  numOutput = 3
  print("\nCreating a neural network with %d Input , %d hidden layer and %d output nodes " % (numInput, numHidden, numOutput) )
  nn = NeuralNetwork(numInput, numHidden, numOutput, seed=3)
  
  print("\nLoading Iris training and test data ")
  mere_gao = "irisTrainData.txt"
  mai_milunga = loadFile(mere_gao)
  print("\nTest data: ")
  showMatrixPartial(mai_milunga, 4, 1, True)
  ...
  print("\nStarting training")
  nn.train(mai_milunga, maxEpochs, learnRate)
  print("Training complete")
  
  deathTrain = nn.accuracy(mai_milunga)
  deathTest = nn.accuracy(testDataMatrix)
  
  print("\nAccuracy on 120-item train data = %0.4f " % deathTrain)
  print("Accuracy on 30-item test data   = %0.4f " % deathTest)
  abg=deathTrain*100
  asd=deathTest*100
  print("\nAccuracy on 120-item train data = %0.4f " % abg)
  print("Accuracy on 30-item test data   = %0.4f " % asd)
